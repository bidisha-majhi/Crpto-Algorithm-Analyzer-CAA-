...
class CSVGenerator:
     ...
     def __init__(self, output_csv = None):
          if output_csv:
               ...
               self.df = pd.DataFrame(columns = ['Algorithm', 'File Size', 'Mode', 'Time Taken', 'Energy Consumed'])
     
     def read_csv(self, output_csv):
          df = pd.read_csv("results/"+output_csv, header=0, index_col=0)
          return df

     def add_or_update_value(self, algorithm, file_size, mode, time_taken, energy_consumed, index = None):
          print(f"Added Some Value in index {index or len(self.df.index)}...")
          self.df.loc[index or len(self.df.index)] = [algorithm, file_size, mode, time_taken, energy_consumed]
...
if __name__ == "__main__":
     ...
     time_taken = 10
     energy_consumed = 10
     #a.add_or_update_value(algorithm, file_size, mode, time_taken, energy_consumed)
